...
file_size_check_arg_mb_nm_opi__ham__mul() {
    ...
    echo -e "${CYAN}--- file_size_check_arg_mb_nm_opi__ham__mul() file://${PLT_PATH}/.d/.mul/nm_opi_/ham/file_size_check_arg_mb_nm_opi__ham__mul.sh ---${NORMAL}" #sistem info mesage
    echo -e "${CYAN}--- MAIN: inner proc nm_opi_ ---${NORMAL}"                                                                                                                       #sistem info mesage
    #{body}

    if [ -z "${check_size}" ]; then
        plt_info "\$1 mast be value of mb check size : args NOT_DEFINE : return 1"
        return 1
    fi

    if [ $(num_01 ${check_size}) -eq 1 ]; then
        plt_info "\$1 mast be value of mb check size : \$1 not NOT_NUMBER : return 1"
        return 1
    fi
