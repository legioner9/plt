...
//----------------------------------------------------
	...
	public function CreateDB( $sort = "", $order = "", $query = "" ){
		...
		if( empty( $query ) )
			{
				$query = "SELECT ".PREFIX."_banki_comments.id, banki_id, del, author_id, answer, author, text, otziv, name_otziv, tit_otziv, ip, date, approve=1, ".PREFIX."_banki_comments.email as com_email, ".USERPREFIX."_users.email, news_num, comm_num, user_group, reg_date, signature, foto, fullname, land, xfields, name FROM ".PREFIX."_banki_comments LEFT JOIN ".USERPREFIX."_users ON ".PREFIX."_banki_comments.author_id=".USERPREFIX."_users.user_id WHERE ".PREFIX."_banki_comments.approve = 1 AND ".PREFIX."_banki_comments.banki_id = '{$this->NewsID}' {$sort} {$order}";
			}
...
//-----------------------------------------------
	...
	private function echo_comm( $id, $nbsp, $i = 1 ){
		...
		*********************************************/
		...
		if( $del ) $text = "<span class=\"banki-comment-del\">Этот отзыв был удалён...</span>";
		...
		if( $this->AllowAdd && !$del )
			{
				...
				$tpl->set( "[fast]", "<a onmouseover=\"dle_copy_quote( '{$FastName} ');\" href=\"#\" onclick=\"BankiQuoteSelectedText( '{$FastName}' ); return false;\">" );
				$tpl->set( "[/fast]", "</a>" );
				
				$tpl->set( "{comm-answer}", "<span id=\"button_answer_to_{$id}\"><a href=\"#\" onclick=\"BankiAnswerComment( '{$id}' ); return false;\">Ответить</a></span>" );
				$tpl->set( "[comm-answer]", "<span id=\"button_answer_to_{$id}\"><a href=\"#\" onclick=\"BankiAnswerComment( '{$id}' ); return false;\">" );
		...
		if( $is_logged && ( ( $member_id['name'] == $UserName && $is_register ) || $this->AllowEdit === true ) && !$del )
			{
				$tpl->set( "[com-edit]", "<a onclick=\"BankiEditCom( '{$id}' ); return false;\" href=\"javascript:void(0);\">" );
				$tpl->set( "[/com-edit]", "</a>" );
			} 
				else
			{
		...
		if( $is_logged && ( ( $member_id['name'] == $UserName && $is_register ) || $this->AllowEdit === true ) && !$del )
			{
				$tpl->set( "[com-del]", "<a href=\"javascript:void(0);\" onclick=\"BankiDelCom( '{$id}', '{$this->NewsID}', '{$otziv}' ); return false;\">" );
				$tpl->set( "[/com-del]", "</a>" );
			} 
				else
			{
...
//-----------------------------------------------
	...
	private function CNPage( $page, $title = "", $return = "" ){
		...
		if( $this->Ajax ) $OnClick = "onclick=\"BankiComPage( '{$page}', '{$this->NewsID}', '{$this->URLAjax}' ); return false;\"";
		if( $title ) $page = $title;
	...
	public function BuildNavigation( $UrlOff = "", $UrlOn = "" ){
		...
		if( $this->Ajax ) 
			{
				$OnClickPrev = "onclick=\"BankiComPage( '".( $this->Page - 1 )."', '{$this->NewsID}', '{$this->URLAjax}' ); return false;\"";
				$OnClickNext = "onclick=\"BankiComPage( '".( $this->Page + 1 )."', '{$this->NewsID}', '{$this->URLAjax}' ); return false;\"";
			}
