...
if( $NewsList['cache'] ) $tpl->result[ $NewsList['tpl_compile'] ] = $Banki->OpenCache( $NewsList['cashe_folder'], $NewsList['cashe_file'], intval( $_REQUEST['page'] ) );
if( !$tpl->result[ $NewsList['tpl_compile'] ] )
	{
		...
		if( $posts > 0 )
			{
				...
				while( $row = $db->get_row( $result ) )
					{
						...
						if( $Banki->Config['short_echo_allowhtml'] != "yes" ) $text = strip_tags( $text, "<br>" );
						if( strlen( $text ) > $Banki->Config['short_echo_max_strlen'] ) $text = substr( $text, 0, $Banki->Config['short_echo_max_strlen'] )."..";
						...
						$tpl->set( "{gcategory}", $Banki->ReturnLinkHrefGCategory( $category ) );
						$tpl->set( "{title}", $title );
						...
						if( $NewsList['banners'] !== false ) $tpl->copy_template .= $Banki->SetBanners( "short", $i );
				...
				if( $NewsList['nav'] !== false ) $Banki->CreateNavigation( $NewsList['nav'], $page, $total, $NewsList['template_nav'], $page_ajax, $NewsList['tpl_compile'] );
		...
		if( $NewsList['cache'] && $Banki->Config['cache'] == "yes" ) $Banki->CreateCache( $NewsList['cashe_folder'], $NewsList['cashe_file'], $tpl->result[ $NewsList['tpl_compile'] ], intval( $_REQUEST['page'] ) );
	}
