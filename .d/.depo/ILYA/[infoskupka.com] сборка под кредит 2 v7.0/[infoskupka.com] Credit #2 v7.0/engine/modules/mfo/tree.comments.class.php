...
class tree_comments {
	...
	public $NoCom = array(
		...
		"template" => "mfo/default/no-otziv.tpl",
		"Load" => true
	);
...
//----------------------------------------------------
	...
	public function CreateDB( $sort = "", $order = "", $query = "" ){
		...
		$this->NewsID = $id;
		if( !$sort ) $sort = PREFIX."_mfo_comments.id";
		$sort = "ORDER BY {$sort}";
		
		if( empty( $query ) )
			{
				$query = "SELECT ".PREFIX."_mfo_comments.id, mfo_id, del, author_id, answer, author, text, otziv, name_otziv, tit_otziv, ip, date, approve=1, ".PREFIX."_mfo_comments.email as com_email, ".USERPREFIX."_users.email, news_num, comm_num, user_group, reg_date, signature, foto, fullname, land, xfields, name FROM ".PREFIX."_mfo_comments LEFT JOIN ".USERPREFIX."_users ON ".PREFIX."_mfo_comments.author_id=".USERPREFIX."_users.user_id WHERE ".PREFIX."_mfo_comments.approve = 1 AND ".PREFIX."_mfo_comments.mfo_id = '{$this->NewsID}' {$sort} {$order}";
			}
...
//-----------------------------------------------
	...
	private function echo_comm( $id, $nbsp, $i = 1 ){
		...
		*********************************************/
		...
		if( $del ) $text = "<span class=\"mfo-comment-del\">Этот отзыв был удалён...</span>";
		...
		if( $this->AllowAdd && !$del )
			{
				...
				$tpl->set( "[fast]", "<a onmouseover=\"dle_copy_quote( '{$FastName} ');\" href=\"#\" onclick=\"MfoQuoteSelectedText( '{$FastName}' ); return false;\">" );
				$tpl->set( "[/fast]", "</a>" );
				
				$tpl->set( "{comm-answer}", "<span id=\"button_answer_to_{$id}\"><a href=\"#\" onclick=\"MfoAnswerComment( '{$id}' ); return false;\">Ответить</a></span>" );
				$tpl->set( "[comm-answer]", "<span id=\"button_answer_to_{$id}\"><a href=\"#\" onclick=\"MfoAnswerComment( '{$id}' ); return false;\">" );
		...
		if( $is_logged && ( ( $member_id['name'] == $UserName && $is_register ) || $this->AllowEdit === true ) && !$del )
			{
				$tpl->set( "[com-edit]", "<a onclick=\"MfoEditCom( '{$id}' ); return false;\" href=\"javascript:void(0);\">" );
				$tpl->set( "[/com-edit]", "</a>" );
			} 
				else
			{
		...
		if( $is_logged && ( ( $member_id['name'] == $UserName && $is_register ) || $this->AllowEdit === true ) && !$del )
			{
				$tpl->set( "[com-del]", "<a href=\"javascript:void(0);\" onclick=\"MfoDelCom( '{$id}', '{$this->NewsID}', '{$otziv}' ); return false;\">" );
				$tpl->set( "[/com-del]", "</a>" );
			} 
				else
			{
...
//-----------------------------------------------
	...
	private function CNPage( $page, $title = "", $return = "" ){
		...
		if( $this->Ajax ) $OnClick = "onclick=\"MfoComPage( '{$page}', '{$this->NewsID}', '{$this->URLAjax}' ); return false;\"";
		if( $title ) $page = $title;
	...
	public function BuildNavigation( $UrlOff = "", $UrlOn = "" ){
		...
		if( $this->Ajax ) 
			{
				$OnClickPrev = "onclick=\"MfoComPage( '".( $this->Page - 1 )."', '{$this->NewsID}', '{$this->URLAjax}' ); return false;\"";
				$OnClickNext = "onclick=\"MfoComPage( '".( $this->Page + 1 )."', '{$this->NewsID}', '{$this->URLAjax}' ); return false;\"";
			}
