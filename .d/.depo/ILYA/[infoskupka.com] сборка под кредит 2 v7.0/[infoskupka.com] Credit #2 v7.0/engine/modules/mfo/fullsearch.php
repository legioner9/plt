...
if( isset( $_REQUEST['submit'] ) )
	{
		...
		$mfo_type = intval( $_REQUEST['mfo_type'] );
		...
		$SearchTitle = htmlspecialchars( $Mfo->FilterSearchText( $_REQUEST['title'] ), ENT_QUOTES, $config['charset'] );
		$SearchText = htmlspecialchars( $Mfo->FilterSearchText( $_REQUEST['text'] ), ENT_QUOTES, $config['charset'] );
		$thread_cats = intval( $_REQUEST['thread_cats'] );
		$Mfo->LoadListCity( $country );
		...
		if( $category )
			{
				if( $thread_cats )
					{
						$ThreadCatForSql = $Mfo->SqlThreadCat( $category );
		...
		$PageLink = "/index.php?do=mfo&action=fullsearch&title=".urlencode( $SearchTitle )."&text=".urlencode( $SearchText )."&date={$date}&country={$country}&city={$city}&category={$category}&thread_cats={$thread_cats}&mfo_type={$mfo_type}&price_ot={$SearchPriceOt}&price_do={$SearchPriceDo}&currency={$currency}&on_page={$SearchOnPage}&submit=1&page={page}";
	}
		else
	{
...
$SelectDate = $Mfo->SelectList( array( 0 => "Без разницы", 1 => "За сегодня", 2 => "За неделю", 3 => "За месяц" ), $date );
$SelectMfoType = $Mfo->ReturnSelectMfoType( $mfo_type );
$SelectCurrency = $Mfo->ReturnSelectCurrency( $currency );
$SelectCategory = $Mfo->SelectCategory( $category, "", "", "", true );
$SelectCountry = $Mfo->ReturnSelectCountry( $country );
$SelectCity = $Mfo->ReturnSelectCity( $country, $city );

$tpl->Load_Template( "/mfo/main/full.search.tpl" );
...
$tpl->set( "{mfo_type}", "<select name=\"mfo_type\">{$SelectMfoType}</select>" );
...
$tpl->copy_template = "<form method=\"post\" action=\"\">{$tpl->copy_template}<input type=\"hidden\" name=\"do\" value=\"mfo\" /><input type=\"hidden\" name=\"action\" value=\"fullsearch\" /></form>";
$tpl->compile( "mfo_search" );
$tpl->clear();
...
if( $SearchSQL )
	{
		$NewsList = array(
			...
			"cashe_file"	=> "",
			"cache"			=> false,
			"sql_count"		=> "SELECT COUNT(*) as count FROM ".PREFIX."_mfo_post WHERE approve='1' {$SearchSQL}",
			"sql_query"		=> "SELECT * FROM ".PREFIX."_mfo_post WHERE approve='1' {$SearchSQL} ORDER BY date DESC {limit}",
			"num"			=> $SearchOnPage,
			"nav"			=> $PageLink,
			"search_info"	=> true,
...
$tpl->result['mfo_content'] = $tpl->result['mfo_search'].$tpl->result['mfo_content'];

// Разрешаем вывод формы поиска
$MfoLoadMainTpl = true;
$MfoAllowSearch = false;
