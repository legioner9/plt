...
if( ( $is_logged || $Mfo->Config['com_anonim'] == "yes" ) && isset( $_POST['addcomment'] ) )
	{
		$error = "";
		if( intval( $Mfo->Config['com_antispam'] ) > 0 && $_SESSION['mfo_add_com_time'] )
			{
				$limit_sec = intval( $proposal_config['com_antispam'] );
				$this_time = ( time () + ( $config['date_adjust'] * 60 ) );
				$period = $this_time - $_SESSION['mfo_add_com_time'];
				if( $period < $limit_sec ) 
					$error = "Сработала антиспам система, следующий отзыв вы сможете добавить через ".( $limit_sec - $period )." сек.";
				else
					@session_unregister( "mfo_add_com_time" );
			}
		
		$GroupComCaptcha = explode( ",", $Mfo->Config['com_groups_captcha'] );
		...
		if( empty( $error ) )
			{
				$ok_add_com = true;
				$db->query( "INSERT INTO ".PREFIX."_mfo_comments (author,author_id,email,text,otziv,name_otziv,tit_otziv,date,mfo_id,del,ip,answer) VALUES('$author','$author_id','$email','$comment','$otziv','$name_otziv','$tit_otziv','$thisdate','$id','0','$_IP','$answer_id')" );
				$db->query( "UPDATE ".PREFIX."_mfo_post SET comm_num=comm_num+1 WHERE id='{$id}'" );
				
				
				if( in_array( "on_create", $Mfo->AllowSendEmail ) )
					{
						$rowUser = $db->super_query( "SELECT `name`,`email` FROM ".USERPREFIX."_users WHERE `user_id`='{$row['author_id']}'" );
						if( $rowUser['email'] )
							{
								$Mfo->SendEmail( "send_on_comment", "Оставлен отзыв о МФО", "", $rowUser['email'], array(
									"title"			=> stripslashes( $row['title'] ),
									"link"			=> "https://".$_SERVER['HTTP_HOST'].$Mfo->ReturnLinkPost( $row['id'], $row['alt_name'], $row['category'] ),
									"user"			=> stripslashes( $rowUser['name'] ),
								) );
							}
					}
				
				@session_register( "mfo_add_com_time" );
				$_SESSION['mfo_add_com_time'] = ( time () + ( $config['date_adjust'] * 60 ) );
