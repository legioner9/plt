...
HTML;
	...
	$row = $db->super_query( "SELECT id, banki_id, date, author, text, approve FROM " . PREFIX . "_{$allowed_areas[$area]['comments_table']} WHERE id = '{$id}'" );
	...
	if( !$row['approve'] ) $db->query( "UPDATE " . PREFIX . "_banki_post SET comm_num=comm_num+1 WHERE id='{$row['post_id']}'" );
