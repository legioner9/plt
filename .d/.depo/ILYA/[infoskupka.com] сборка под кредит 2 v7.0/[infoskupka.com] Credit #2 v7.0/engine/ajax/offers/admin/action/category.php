...
=====================================================
 Назначение: массовые действия с категориями
=====================================================
*/
								...
								foreach( $Sort as $id => $value )
									{
										...
										if( $id )
											{
												$db->query( "UPDATE ".PREFIX."_offers_options SET `sort`='{$value}' WHERE `id`='{$id}' LIMIT 1" );
											}
									}
								...
								foreach( $CheckCategory as $id )
									{
										...
										if( $id )
											{
												$CountRow = $db->super_query( "SELECT COUNT(*) AS count FROM ".PREFIX."_offers_post WHERE category='{$id}'" );
												$Count = intval( $CountRow['count'] );
												for( $i = 0; $i < $Count; $i++ )
													{
														$Offers->AddPostCategory( $NewCategory );
														$Offers->DelPostCategory( $id );
													}
												$db->query( "UPDATE ".PREFIX."_offers_post SET `category`='{$NewCategory}' WHERE `category`='{$id}'" );
...
HTML;
								...
								else
							{
								tpl_table_open($navigation);
								tableheader( "Список категорий &raquo; Массовые действия" );
								$SelectCategory = $Offers->SelectCategory( "", $CheckCategory );
echo <<<HTML

<form method="post" action="" name="OffersMassAction" onsubmit="formSubmit( 'OffersMassAction', 'action.php', 'category', 'ajax.gif', 'UpdateStatus', 'UpdateStatus', UpdateButton ); return false;">
	<div style="padding: 25px; text-align: center;">
		Вы выбрали  <strong>({$CountCheck})</strong> категорий, выберите в какую категорию переместить их офферы:<br /><br />
