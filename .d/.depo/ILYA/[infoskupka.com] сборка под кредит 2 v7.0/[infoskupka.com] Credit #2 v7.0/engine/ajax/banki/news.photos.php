...
if( $postID )
	{
		$row = $db->super_query( "SELECT `id`,`author`,`author_id`,`photos`,`approve` FROM ".PREFIX."_banki_post WHERE `id`='{$postID}'" );
		if( !$row['id'] ) echoReturn( "Объявление не обнаружено!" );
		if( !$Banki->AllowEditPost( $row['author_id'], $row['approve'] ) ) echoReturn( "Вы не можете редактировать данное объявление!" );
	}
		else
	{
		if( !$Banki->AllowAddPost() ) echoReturn( "Вы не можете добавлять объявления!" );
	}
...
$ThumbPhotoSize = $Banki->Config['add_post_thumb_photo']; // Размер уменьшенной картинки в px
$MaxPhotoKB = $Banki->Config['add_post_size_photo'] * 1024; // Максимальное размер загружаемой фотки в байтах (при умножении на 1024 получается в КБ)
$AllowedExtensions = explode( ",", $Banki->Config['add_post_photo_type'] ); // Разрешённые типы картинок
...
if( $_REQUEST['upload'] == 1 )
	{
		$countPhoto = $Banki->CountImages( $row['photos'] );
		if( $countPhoto >= $Banki->Config['add_post_max_photo'] && !$admin ) echoReturn( "Вы достигли лимита изображений для одного объявления, удалите существующие, чтобы загрузить новые!" );
		
		$FolderPrefix = date( "Y-m" );
		$Folder = ROOT_DIR."/uploads/banki/post/{$FolderPrefix}";
						...
						if( !file_exists( ROOT_DIR.$ImageFile ) )
							{
								...
								if( $res ) 
									{											
										...
										$Banki->AddPhotosInfoDB( $Photo, $FolderPrefix, $row );
										
										//	Настройка шаблонов
										
										if( $admin )
											{
												$PhotoEcho = "{photo}[del]<img src=\"engine/inc/banki/style/images/delete.png\" class=\"del\" alt=\"Удалить\" />[/del]";
												$PhotoEcho = str_replace( "{photo}", "<a href=\"/uploads/banki/post/{$FolderPrefix}/{$Photo}\" target=\"_blank\"><img src=\"/uploads/banki/post/{$FolderPrefix}/thumbs/{$Photo}\" alt=\"\" /></a>", $PhotoEcho );
												
												$PhotoEcho = str_replace( "[del]", "<a href=\"javascript:void(0);\" onclick=\"DelBankiPhoto( '{$Photo}', '{$row['id']}' ); return false;\">", $PhotoEcho );
											...
											{
												$tpl->Load_Template( "banki/main/add_photos/photo.list.tpl" );
												$tpl->copy_template = "<div id=\"{$Photo}\">".$tpl->copy_template."</div>";
												$tpl->set( "{photo}", "<a href=\"/uploads/banki/post/{$FolderPrefix}/{$Photo}\" target=\"_blank\" onclick=\"return hs.expand(this)\"><img src=\"/uploads/banki/post/{$FolderPrefix}/thumbs/{$Photo}\" alt=\"\" /></a>" );
												$tpl->set( "{photo:thumb}", "/uploads/banki/post/{$FolderPrefix}/thumbs/{$Photo}" );
												$tpl->set( "{photo:url}", "/uploads/banki/post/{$FolderPrefix}/{$Photo}" );
												$tpl->set( "{photo:name}", $Photo );
														
												$tpl->set( "[del]", "<a href=\"javascript:void(0);\" onclick=\"DelBankiPhoto( '{$Photo}', '{$row['id']}' ); return false;\">" );
												$tpl->set( "[/del]", "</a>" );
				...
				for( $i = 0; $i < count( $file ); $i++ )
					{
						...
						$PhotoDel = ROOT_DIR."/uploads/banki/post/{$image[1][1]}/{$image[1][0]}";
						$PhotoThumbsDel = ROOT_DIR."/uploads/banki/post/{$image[1][1]}/thumbs/{$image[1][0]}";
						unset( $file[ $i ] );
					}	
				...
				for( $i = 0; $i < count( $photos ); $i++ )
					{
						...
						$PhotoDel = ROOT_DIR."/uploads/banki/post/{$img[1]}/{$img[0]}";
						$PhotoThumbsDel = ROOT_DIR."/uploads/banki/post/{$img[1]}/thumbs/{$img[0]}";
						unset( $photos[ $i ] );
					}
