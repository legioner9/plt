								...
								foreach( $CheckCategory as $id )
									{
										...
										if( $id )
											{
												$CountRow = $db->super_query( "SELECT COUNT(*) AS count FROM ".PREFIX."_banki_post WHERE category='{$id}'" );
												$Count = intval( $CountRow['count'] );
												for( $i = 0; $i < $Count; $i++ )
													{
														$Banki->AddPostCategory( $NewCategory );
														$Banki->DelPostCategory( $id );
													}
												$db->query( "UPDATE ".PREFIX."_banki_post SET `category`='{$NewCategory}' WHERE `category`='{$id}'" );
...
HTML;
								...
								else
							{
								tpl_table_open($navigation);
								tableheader( "Список категорий &raquo; Массовые действия" );
								$SelectCategory = $Banki->SelectCategory( "", $CheckCategory );
echo <<<HTML

<form method="post" action="" name="BankiMassAction" onsubmit="formSubmit( 'BankiMassAction', 'action.php', 'category', 'ajax.gif', 'UpdateStatus', 'UpdateStatus', UpdateButton ); return false;">
	<div style="padding: 25px; text-align: center;">
