...
$allowed_areas = array(
					...
					'news' => array (
									'comments_table' => 'offers_comments',
									),
...
if( $_REQUEST['action'] == "edit" ) {
	$row = $db->super_query( "SELECT id, date, author, text, tit_otziv FROM " . PREFIX . "_{$allowed_areas[$area]['comments_table']} where id = '$id'" );
...
HTML;
	...
	$row = $db->super_query( "SELECT id, offers_id, date, author, text, approve FROM " . PREFIX . "_{$allowed_areas[$area]['comments_table']} WHERE id = '{$id}'" );
	...
	if( !$row['approve'] ) $db->query( "UPDATE " . PREFIX . "_offers_post SET comm_num=comm_num+1 WHERE id='{$row['post_id']}'" );
