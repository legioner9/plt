...
require_once( ENGINE_DIR."/inc/banki/init.php" );

$do = $_REQUEST['do'];
$action = $_REQUEST['action'];


$navigation = ($do != '') ? '<a href="'.$config['admin_path'].'?mod=banki">Банки 6.0</a>' : 'Банки 6.0';
...
switch ($do) {
    ...
    case "clearcache":
        faq_clearcache(1);
        msg("info", $faq_lang['info'], $faq_lang['cache_success_clean'], "$PHP_SELF?mod=banki");
    break;
    default:
...
function BankiGroups( $selected, $visitor = "yes", $all = false ) {
	global $user_group;
...
<div style="padding-top:10px; padding-bottom:10px;">
	<div style="text-align: center;" class="navigation"><strong>DLE Banki</strong> Powered <strong>By radimid.ru </a></strong> for DataLife Engine</a></div>
</div>
{$skin_footer}
HTML;
...
function banki_sort( $url, $n, $ajax="" ){

   // Делаем переменные
	$img_url = "engine/inc/banki/style/images/arrow";	
...
function Banki_Navigation($url_off,$url_on, $count_page, $page_get, $ajax=""){
  global $config;
...
function SetReferer( $url ){
	@session_register( "AdminbankiReferer" );
	$_SESSION['AdminbankiReferer'] = $url;
}
...
function minimsg( $title, $text, $back = "", $close = true ){
	...
	if( $close )
		{
			$close = "<a class=\"btn bg-teal btn-sm btn-raised position-left legitRipple\" href=\"$PHP_SELF?mod=banki\" />На главную страницу модуля</a>";	
		}
			else
		{
			$close = "";	
