...
class BankiField {
	...
	function ExplodeXfields( $value, $decode = false ){
		...
		$this->BankiXfields = array();
		$value = explode( "|||", $value );
				
		foreach( $value as $ToXfields )
			{
				$ToXfields = explode( "=", $ToXfields );
				$this->BankiXfields[ $ToXfields[0] ] = $this->FilterText( $ToXfields[1], "edit" );
				if( $decode === true ) $this->BankiXfields[ $ToXfields[0] ] =  htmlspecialchars_decode( $this->BankiXfields[ $ToXfields[0] ] );
			}		
	}
						...
						if( method_exists( $this, $MethodName ) )
							{
								...
								$ForReturn = $template;
								$TypeForm = $this->$MethodName( $row[15], $this->BankiXfields[ $row[1] ], $row[1] );
								...
								$ForReturn = str_replace( "{id}", "id=\"BankiField_{$i}\"", $ForReturn );
								...
								if( !$AllowEdit || !$AllowCategory )
									{
										...
										$TypeForm = $this->BankiXfields[ $row[1] ];
									}
										else
									{
										// Если разрешено редактировать
										$TypeForm = $this->$MethodName( $row[1], $row[15], $this->BankiXfields );
		...
		if( in_array( $Type, $this->AllowType ) && method_exists( $this, $method_name ) )
			{
				...
				$FieldValue = $this->BankiXfields[ $TagValue[0] ];
	...
	function ParseTags( $value, $xfields ){
		...
		$value = preg_replace_callback( "#\[Bankifield:(.+?)\](.+?)\[/Bankifield:(.+?)\]#is", function( $matches ){
			return $this->ParseTagsBlock( $matches[1], $matches[2] );
		}, $value );
		$value = preg_replace_callback( "#\[not-Bankifield:(.+?)\](.+?)\[/not-Bankifield:(.+?)\]#is", function( $matches ){
			return $this->ParseTagsBlock( $matches[1], $matches[2], 'block' );
		}, $value );
		$value = preg_replace_callback( "#{Bankifield:(.+?)}#is", function( $matches ){
		...
		foreach( $this->CategoryJS as $Category => $FieldsID )
			{
				...
				$Fields[] = "BankiFields['{$Category}'] = [".implode( ",", $FieldsID )."];"; 
			}
		
		$Fields = implode( "\n", $Fields );
		$Category = implode( ",", $CategoryArray );
		$return = "var BankiFieldsCat = [{$Category}];\nvar BankiFields = new Array();\n{$Fields}";
		return $return;		
	}
...
HTML;
	...
	function ShowForm_mini_text( $options, $value = "", $alt_name ){
		...
		return "<input type=\"text\" name=\"field[{$alt_name}]\" value=\"{$value}\" class=\"Bankifield Bankifield-{$alt_name} form-control\" style=\"width: {$width}px;\" />";
...
HTML;
	...
	function ShowForm_text( $options, $value = "", $alt_name ){
		...
		return "<textarea name=\"field[{$alt_name}]\" class=\"Bankifield  Bankifield-{$alt_name}\" style=\"width: 600px; height: 200px;\">{$value}</textarea>";
...
HTML;
	...
	function ShowForm_select( $options, $value = "", $alt_name ){
		...
		$return = "<select name=\"field[{$alt_name}]{$AllowArray}\" class=\"Bankifield Bankifield-{$alt_name}\" style=\"{$Style}\" {$Multiple}>{$return}</select>";
		return $return;
	}
...
HTML;
	...
	function ShowForm_radio( $options, $value = "", $alt_name ){
		...
		switch( $options['show'] ){
			...
			case "br":
				$template = "{value}<br />";
				$MainTeplate = "<span class=\"Bankifield-radio Bankifield-radio-{$alt_name}\">{content}</span>";
				break;
				
			case "li":
				$template = "<li>{value}</li>";
				$MainTeplate = "<ul class=\"Bankifield-radio Bankifield-radio-{$alt_name}\">{content}</ul>";
				break;
...
HTML;
	...
	function ShowForm_checkbox( $options, $value = "", $alt_name ){
		...
		switch( $options['show'] ){
			...
			case "br":
				$template = "{value}<br />";
				$MainTeplate = "<span class=\"Bankifield-checkbox Bankifield-checkbox-{$alt_name}\">{content}</span>";
				break;
				
			case "li":
				$template = "<li>{value}</li>";
				$MainTeplate = "<ul class=\"Bankifield-checkbox Bankifield-checkbox-{$alt_name}\">{content}</ul>";
				break;
...
<span class="Bankifield-file Bankifield-file-{$alt_name}">
								...
								if( in_array( strtolower( $image_type ), $AllowType ) )
									{
										...
										if( is_uploaded_file( $image_upload ) )
											{
												require_once( ENGINE_DIR."/inc/banki/makethumb.php" );
