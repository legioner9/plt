...
/*
 ...
 Файл: field.class.php
=====================================================
 Данный код защищен авторскими правами
=====================================================
 Назначение: класс дополнительных полей
=====================================================
*/
...
class OffersField {
	...
	function SortField() {
		uasort( $this->DB, "OffersSortField" );
	}
	...
	function ExplodeXfields( $value, $decode = false ){
		...
		$this->OffersXfields = array();
		$value = explode( "|||", $value );
				
		foreach( $value as $ToXfields )
			{
				$ToXfields = explode( "=", $ToXfields );
				$this->OffersXfields[ $ToXfields[0] ] = $this->FilterText( $ToXfields[1], "edit" );
				if( $decode === true ) $this->OffersXfields[ $ToXfields[0] ] =  htmlspecialchars_decode( $this->OffersXfields[ $ToXfields[0] ] );
			}		
	}
						...
						if( method_exists( $this, $MethodName ) )
							{
								...
								$ForReturn = $template;
								$TypeForm = $this->$MethodName( $row[15], $this->OffersXfields[ $row[1] ], $row[1] );
								...
								$ForReturn = str_replace( "{id}", "id=\"OffersField_{$i}\"", $ForReturn );
								...
								if( !$AllowEdit || !$AllowCategory )
									{
										...
										$TypeForm = $this->OffersXfields[ $row[1] ];
									}
										else
									{
										// Если разрешено редактировать
										$TypeForm = $this->$MethodName( $row[1], $row[15], $this->OffersXfields );
		...
		if( in_array( $Type, $this->AllowType ) && method_exists( $this, $method_name ) )
			{
				...
				$FieldValue = $this->OffersXfields[ $TagValue[0] ];
	...
	function ParseTags( $value, $xfields ){
		...
		$value = preg_replace_callback( "#\[Offersfield:(.+?)\](.+?)\[/Offersfield:(.+?)\]#is", function( $matches ){
			return $this->ParseTagsBlock( $matches[1], $matches[2] );
		}, $value );
		$value = preg_replace_callback( "#\[not-Offersfield:(.+?)\](.+?)\[/not-Offersfield:(.+?)\]#is", function( $matches ){
			return $this->ParseTagsBlock( $matches[1], $matches[2], 'block' );
		}, $value );
		$value = preg_replace_callback( "#{Offersfield:(.+?)}#is", function( $matches ){
		...
		foreach( $this->CategoryJS as $Category => $FieldsID )
			{
				...
				$Fields[] = "OffersFields['{$Category}'] = [".implode( ",", $FieldsID )."];"; 
			}
		
		$Fields = implode( "\n", $Fields );
		$Category = implode( ",", $CategoryArray );
		$return = "var OffersFieldsCat = [{$Category}];\nvar OffersFields = new Array();\n{$Fields}";
		return $return;		
	}
...
HTML;
	...
	function ShowForm_mini_text( $options, $value = "", $alt_name ){
		...
		return "<input type=\"text\" name=\"field[{$alt_name}]\" value=\"{$value}\" class=\"Offersfield Offersfield-{$alt_name} form-control\" style=\"width: {$width}px;\" />";
...
HTML;
	...
	function ShowForm_text( $options, $value = "", $alt_name ){
		...
		return "<textarea name=\"field[{$alt_name}]\" class=\"Offersfield  Offersfield-{$alt_name}\" style=\"width: 600px; height: 200px;\">{$value}</textarea>";
...
HTML;
	...
	function ShowForm_select( $options, $value = "", $alt_name ){
		...
		$return = "<select name=\"field[{$alt_name}]{$AllowArray}\" class=\"Offersfield Offersfield-{$alt_name}\" style=\"{$Style}\" {$Multiple}>{$return}</select>";
		return $return;
	}
...
HTML;
	...
	function ShowForm_radio( $options, $value = "", $alt_name ){
		...
		switch( $options['show'] ){
			...
			case "br":
				$template = "{value}<br />";
				$MainTeplate = "<span class=\"Offersfield-radio Offersfield-radio-{$alt_name}\">{content}</span>";
				break;
				
			case "li":
				$template = "<li>{value}</li>";
				$MainTeplate = "<ul class=\"Offersfield-radio Offersfield-radio-{$alt_name}\">{content}</ul>";
				break;
...
HTML;
	...
	function ShowForm_checkbox( $options, $value = "", $alt_name ){
		...
		switch( $options['show'] ){
			...
			case "br":
				$template = "{value}<br />";
				$MainTeplate = "<span class=\"Offersfield-checkbox Offersfield-checkbox-{$alt_name}\">{content}</span>";
				break;
				
			case "li":
				$template = "<li>{value}</li>";
				$MainTeplate = "<ul class=\"Offersfield-checkbox Offersfield-checkbox-{$alt_name}\">{content}</ul>";
				break;
...
<span class="Offersfield-file Offersfield-file-{$alt_name}">
								...
								if( in_array( strtolower( $image_type ), $AllowType ) )
									{
										...
										if( is_uploaded_file( $image_upload ) )
											{
												require_once( ENGINE_DIR."/inc/offers/makethumb.php" );
