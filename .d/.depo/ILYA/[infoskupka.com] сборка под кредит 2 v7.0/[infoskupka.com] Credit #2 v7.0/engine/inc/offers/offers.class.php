...
class Offers {
	...
	var $Config = array();
	var $AllowOptionsType = array( "category", "country", "city", "currency", "offers_type", "banners", "blocks" );
	var $Field = false;
	...
	function Start(){
		...
		require( ENGINE_DIR."/data/offers.config.php" );
		include( ENGINE_DIR."/data/offers/cron.php" );
		
		$this->Config = $OffersConfig;
		$this->Cron = $OffersCronConfig;
		$this->CacheDir = ENGINE_DIR."/data/offers/cache/";
		...
		foreach( $this->AllowOptionsType as $type )
			{
				if( !file_exists( ENGINE_DIR."/data/offers/{$type}.php" ) ) $this->CreateDB();
				include_once( ENGINE_DIR."/data/offers/{$type}.php" );
			}
	...
	function StartField( $Necessarily = false ){
		...
		if( ( $this->Field === false && $this->Config['field_on'] == "on" ) || ( $Necessarily === true ) )
			{
				include( ENGINE_DIR."/inc/offers/field.class.php" );
				$this->Field = new OffersField( ENGINE_DIR."/data/offers/fields.offers.db" );
			}
	}
	...
	function SaveConfig( $Opt = array(), $LastOpt, $NameOpt, $File, $title = "(info)", $Ajax = false ){
	...
	function CreateDB(){
		...
		foreach( $this->AllowOptionsType as $type ) @unlink( ENGINE_DIR."/data/offers/{$type}.php" );
		$this->ClearCache( "../city/" );
		
		$AllowCity = array();
		$result = $db->query( "SELECT * FROM ".PREFIX."_offers_options ORDER BY `sort`" );
		...
		foreach( $this->AllowOptionsType as $type )
			{
				$Text = "<?php\n\nif( !defined( \"DATALIFEENGINE\" ) ) die( \"Hacking attempt!\" );\n\n".$AllowType[ $type ]."?>";
				$fp = fopen( ENGINE_DIR."/data/offers/{$type}.php", "w+" );
				fwrite( $fp, $Text );
				fclose( $fp );
			}
		...
		foreach( $AllowCity as $country_id => $row )
			{
				...
				$fp = fopen( ENGINE_DIR."/data/offers/city/city_{$country_id}.php", "w+" );
				fwrite( $fp, $Text );
				fclose( $fp );
			}
		...
		if( !$this->EmailClass )
			{
				...
				include( ENGINE_DIR."/data/offers/tpl.email.php" );
				$this->EmailClass = new dle_mail( $config );
				$this->EmailTpl = $OffersEmail;
	...
	function UpdateViews( $id ){
		...
		if( $this->Config['full_cache_views'] == "yes" )
			{
				$fp = fopen( ENGINE_DIR."/data/offers/post.views.db", "a" );
				fwrite( $fp, "{$id}\n" );
				fclose( $fp );
				@chmod( ENGINE_DIR."/data/offers/post.views.db", 0666 );
			}
				else
			{
				$db->query( "UPDATE ".PREFIX."_offers_post SET `views`=views+1 WHERE `id`='{$id}'" );
			}
	...
	function LastCommentsBlock(){
		...
		if( !$return )
			{
				$template = file_get_contents( $tpl->dir."/offers/main/last.comments.tpl" );
				$result = $db->query( "SELECT ".PREFIX."_offers_comments.id as com_id, ".PREFIX."_offers_comments.offers_id, ".PREFIX."_offers_comments.author_id, ".PREFIX."_offers_comments.author, ".PREFIX."_offers_comments.otz, ".PREFIX."_offers_comments.text, ".PREFIX."_offers_comments.ip, ".PREFIX."_offers_comments.date, ".PREFIX."_offers_post.id, ".PREFIX."_offers_post.title, ".PREFIX."_offers_post.alt_name, ".PREFIX."_offers_post.category FROM ".PREFIX."_offers_comments LEFT JOIN ".PREFIX."_offers_post ON ".PREFIX."_offers_comments.offers_id=".PREFIX."_offers_post.id WHERE ".PREFIX."_offers_comments.del!='1' ORDER BY `date` DESC LIMIT 0," . $maxComm );
				if( $db->num_rows( $result ) > 0 )
					{
						while( $row = $db->get_row( $result ) )
							{
		...
		foreach( $this->DB['category'] as $id => $row )
			{
				...
				if( !$this->CategoryOurIndex[ $id ] )
					{
						$this->EchoCatOk[] = $id;
						$name = htmlspecialchars( stripslashes( $this->DB['category'][ $id ]['title_h'] ), ENT_QUOTES, $config['charset'] );
						$returnJS .= "<div id=\"InCat_{$id}\" style=\"display: none;\"><select name=\"category\" id=\"OffersSelectCategory\" onkeyup=\"CategorySelect();\" onkeydown=\"CategorySelect();\" onchange=\"CategorySelect();\">".$this->SelectCategory( $select, "", -1, $id )."</select></div>";
						
						$selected = ( $this->JsCatSelect === true || $select == $id ) ? "selected=\"selected\"" : "";
						$this->JsCatSelect = false;
						
						$return .= "<option value=\"{$id}\" {$selected}>{$title_h}</option>";
					}
			}
		
		$JSFunction = <<<HTML
		
		var OffersAddStart = true;
		function OffersInCat(){
			var SelectCat = document.getElementById( "mainCat" ).value;
			if( SelectCat > 0 )
				{
					if( document.getElementById( "InCat_" + SelectCat ).innerHTML != "" )
						{
							document.getElementById( "OffersTextForInCat" ).innerHTML = document.getElementById( "InCat_" + SelectCat ).innerHTML;
						}
					
					if( OffersAddStart == true )
						OffersAddStart = false;
					else
						CategorySelect();
					return;
				}
			
			document.getElementById( "OffersTextForInCat" ).innerHTML = "";
		}
...
HTML;
		...
		<select name="MainCategory" id="mainCat" onkeyup="OffersInCat();" onkeydown="OffersInCat();" onchange="OffersInCat();">
			{$return}
		</select>
		<span id="OffersTextForInCat">&nbsp;</span>
...
HTML;
	...
	function ReturnAdminCatForm( $row, $nbsp = 0 ){
		...
		$id = $row['id'];
		$title_h = htmlspecialchars( stripslashes( $row['title_h'] ), ENT_QUOTES, $config['charset'] );
...
$return = <<<HTML
		...
		<tr id="del_Category_{$id}" height="30px"  >
			...
			<td class="hidden-xs">
				{$echoNbsp}
				<a href="$PHP_SELF?mod=offers&do=edit_category&id={$id}" title="Редактировать категорию" >{$title_h}</a>
			...
			<td width="100px" align="center" class="hidden-xs">
				<div class="btn-group">
					...
					<ul class="dropdown-menu text-left">
						<li><a  href="$PHP_SELF?mod=offers&do=edit_category&id={$id}"><i class="icon-desktop"></i> Редактировать</a></li>
...
HTML;
				...
				if( $this->DB['category'][ $id ]['alt_name'] )
					{
						...
						return "/index.php?do=offers&action=offerscat&offerscat={$alt_name}";	
					}
						else
					{
						return "/index.php?do=offers";	
					}
			}
	...
	function ReturnLinkPost( $id, $alt_name, $cat_id ){
		...
		if( $this->AltUrl === true )
				...
				else
			{
				...
				$return = "/index.php?do=offers&offerscat={$alt_name}&action=post&id={$id}";
			}
			
		return $return;
	}
	
	function get_url_offers($id, $cat_info) {
	...
	function ReturnLinkAddPost(){
		...
		if( $this->AltUrl === true )
				...
				else
			{
				...
				$return = "/index.php?do=offers&offerscat={$alt_name}&action=add";
			}
	...
	function ListAllCategory( $MainTemplate, $ThreadTemplate, $Columns = 1, $id = "" ){
		...
		$return = "<table width=\"100%\" class=\"OffersMainCats\"><tr>{$return}</tr></table>";			
		return $return;
	}
		...
		if( !$thread )
			{
				...
				$count_offerss = intval( $this->DB['category'][ $id ]['count_post'] );
				
				$return = str_replace( "{link}", $link, $this->EchoCatMainTemplate );
				$return = str_replace( "{title}", $title, $return );
				$return = str_replace( "{count_offerss}", $count_offerss, $return );
								...
								foreach( $this->CategoryIndex[ $id ] as $NewId )
									{
										...
										$count_offerss = intval( $this->DB['category'][ $NewId ]['count_post'] );
										$Comma[] = "<a href=\"{$link}\">{$title}</a> <span>({$count_offerss})</span>";
									}
								...
								foreach( $this->CategoryIndex[ $id ] as $NewId )
									{
										...
										$ForReturn = str_replace( "{title}", $title, $ForReturn );
										$ForReturn = str_replace( "{count_offerss}", $count_offerss, $ForReturn );
										
										if( $icon )
											{
												$ForReturn = preg_replace( "#\\[icon\\](.+?)\\[/icon\\]#is", "\\1", $ForReturn );
												$ForReturn = str_replace( "{icon}", "{THEME}/offers/style/{$icon}", $ForReturn );
											}
												else
											{
	...
	function SaveOffersOpt( $row, $newRow, $id = "" ){
		global $db;
		...
		$db->query( "UPDATE ".PREFIX."_offers_options SET `options`='{$SqlOpt}' WHERE `id`='{$id}' LIMIT 1" );
	}
		...
		if( $this->DB['category'][ $id ] )
			{
				$title = htmlspecialchars( stripslashes( $this->DB['category'][ $id ]['name'] ), ENT_QUOTES, $config['charset'] );
				$title_h = htmlspecialchars( stripslashes( $this->DB['category'][ $id ]['title_h'] ), ENT_QUOTES, $config['charset'] );
				$opisanie = $this->DB['category'][ $id ]['opisanie'];
				$link = $this->ReturnLinkCategory( $id );
				$this->SpeedBar[] = array(
					"opisanie"		=> $opisanie,
					"title_h"		=> $title_h,
	...
	function CreateNavigation( $url, $page, $total, $tpl = "navigation.tpl", $ajax = "", $result = "offers" ){
		
		if( !$this->Navigation )
			{
				require( ENGINE_DIR."/modules/offers/navigation.class.php" );
				$this->Navigation = new OffersNavigation;
			}
	...
	function NotepadPost( $id, $span = true ){
		...
		if( $is_logged )
			{
				if( $member_id['offers_notepad'] )
					{
						$Notepad = explode( ",", $member_id['offers_notepad'] );
						if( in_array( $id, $Notepad ) )
							{
								$return = "<a href=\"javascript:void(0);\" onclick=\"OffersNotepad( '{$id}', 'del' ); return false;\"return false;\" class='return-to-catalog'>&nbsp;&nbsp;&nbsp;Удалить из избранного</a>";
							}
					}
				
				if( !$return ) $return = "<a href=\"javascript:void(0);\" onclick=\"OffersNotepad( '{$id}', 'add' ); return false;\" class='return-to-catalog'>&nbsp;&nbsp;&nbsp;Добавить в избранное</a>";
				if( $span === true ) $return = "<span id=\"OffersPostNotepad_{$id}\">{$return}</span>";
			}
	...
	function AddPostCategory( $CategoryID ){
		...
		$fp = fopen( ENGINE_DIR."/data/offers/post.cat.count.db", "a" );
	...
	function ParsePhotosNews( $text, $images ){
		...
		if( $images )
			{
				...
				foreach( $images as $img )
					{
						$img = explode( "###", $img );	
						$Photo = "/uploads/offers/post/{$img[1]}/{$img[0]}";
						$PhotoThumb = "/uploads/offers/post/{$img[1]}/thumbs/{$img[0]}";
						
						$hs = "<a href=\"{$Photo}\" target=\"_blank\" onclick=\"return hs.expand(this)\"><img src=\"{$PhotoThumb}\" alt=\"\" /></a>";
						$all .= "<div class=\"OffersPhoto OffersPhoto-{$i}\">{$hs}</div>";
						$allBig .= "<div class=\"OffersPhoto OffersPhoto-{$i}\"><a href=\"{$Photo}\" target=\"_blank\" onclick=\"return hs.expand(this)\"><img src=\"{$Photo}\" alt=\"\" /></a></div>";
		...
		$text = str_replace( "{image:all}", "<div class=\"OffersNotImages\">Нет изображений</div>", $text );
	...
	function OffersNewsImages( $row, $template, $mainTemplate, $array = false, $admin = false ){
				...
				foreach( $imgArray as $img )
					{
						...
						$tpl = str_replace( "[del]", "<a href=\"javascript:void(0);\" onclick=\"DelOffersPhoto( '{$img[0]}', '{$row['id']}' ); return false;\">", $tpl );
		...
		$returnJS = <<<HTML
			...
			<script type="text/javascript" src="/engine/ajax/offers/ajaxupload.js"></script>
			<script language="javascript" type="text/javascript">
				var OffersCountPhotos = {$CountPhotos};
				function OffersUploadPhotos(){
					$(function(){
						var btnUpload = $( "#OffersUploadPhotos" );
						var status = $( "#StatusOffersUploadPhotos" );
						new AjaxUpload( btnUpload, {
							action: "/engine/ajax/offers/news.photos.php",
							name: 	"Filedata",
							...
							onComplete: function( file, response ){
								...
								status.html( "" );
								if( OffersCountPhotos == 0 ) $( "#OffersImages" ).html( "" );
								$( "#OffersImages" ).html( document.getElementById( "OffersImages" ).innerHTML + response );
								OffersCountPhotos++;
							}
						});
				...
				function DelOffersPhoto( Photo, PostID ){
					...
					if( Quest )
						{
							...
							ShowLoading( "" );
							$.ajax({
								url: "/engine/ajax/offers/news.photos.php",
								...
								success: function( data ){
									...
									if( data )
											...
											else
										{
											...
											OffersCountPhotos--;
											if( OffersCountPhotos <= 0 )
												{
													$( "#OffersImages" ).html( "Нет загруженных фотографий" );
												}
										}
								},
								dataType: "html",
...
HTML;
	...
	function CountImages( $images = "" ){
		...
		$return = 0;
		$fileDB = ENGINE_DIR."/data/offers/photos.db";
		$img = file( $fileDB );
		if( count( $img ) > 0 )
			{
		...
		if( $row['id'] )
			{
				...
				$db->query( "UPDATE ".PREFIX."_offers_post SET `photos`='{$photos}' WHERE `id`='{$row['id']}'" );
				return true;
			}
		...
		for( $i = 0; $i < $CountDBPhotos; $i++ )
			{
				...
				if( ( ( $thisTime - $imgTime ) / 60 ) < $Offers->Config['add_post_photo_del'] ) continue;
				
				@unlink( ROOT_DIR."/uploads/offers/post/{$image[1][1]}/{$image[1][0]}" );
				@unlink( ROOT_DIR."/uploads/offers/post/{$image[1][1]}/thumbs/{$image[1][0]}" );
	...
	function ClearCacheCountPost(){
		...
		$FileOpen = ENGINE_DIR."/data/offers/post.cat.count.db";
	...
	function CronUpdate( $Update = array() ){
		...
		$fp = fopen( ENGINE_DIR."/data/offers/cron.php", "w+" );
		fwrite( $fp, "<?php\n\n//Задачи кронов модуля Offers (Powered by  )\n\n\$OffersCronConfig = array(\n" );
	...
	function CleareAllCache(){
		...
		$NotDelFile = array( ".", "..", ".htaccess", "fields.offers.db", "photos.db", "cron.php", "post.cat.count.db", "post.views.db", "tpl.email.php" );
		$dir = opendir( ENGINE_DIR."/data/offers/" );
		while( $file = readdir( $dir ) )
			{
				if( !in_array( $file, $NotDelFile ) )
					{
						@unlink( ENGINE_DIR."/data/offers/".$file );
					}
			}
	...
	function UpdateColor(){
		...
		$thisTime = time() + ( $config['date_adjust'] * 60 );
		$db->query( "UPDATE ".PREFIX."_offers_post SET `color_date`='' WHERE `color_date` < '{$thisTime}'" );
	}
	
	function UpdateDBViews(){
		global $db;
		
		$FileOpen = ENGINE_DIR."/data/offers/post.views.db";
		if( file_exists( $FileOpen ) )
			{
				$IDView = array();
				$File = file( $FileOpen );
				...
				if( count( $IDView ) > 0 )
					{
						...
						$db->query( "UPDATE ".PREFIX."_offers_post SET views = CASE {$SQLIDView} END WHERE id IN ({$AllowID})" );
					}
