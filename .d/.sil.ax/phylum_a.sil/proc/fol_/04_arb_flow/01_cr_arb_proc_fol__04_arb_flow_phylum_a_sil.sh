...
01_cr_arb_proc_fol__04_arb_flow_phylum_a_sil() {
    ...
    echo -e "${BLUE}--- 01_cr_arb_proc_fol__04_arb_flow_phylum_a_sil() file://${PLT_PATH}/.d/.sil.ax/phylum_a.sil/proc/fol_/04_arb_flow/01_cr_arb_proc_fol__04_arb_flow_phylum_a_sil.sh ---${NORMAL}" #sistem info mesage
    ...
    if [ "${crarb_:-0}" -eq 1 ]; then
        ...
        if ! cp -rv "${sil_}"/arb/name_ramus.ram ${arb_}; then
            plt_exit "in fol_() : EXEC_FAIL : cp -rv ${sil_}/arb/name_ramus.ram ${arb_} return 1"
            return 1
        fi
