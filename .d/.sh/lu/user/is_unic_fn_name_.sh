#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/is_unic_fn_name_.sh"
...
is_unic_fn_name_() {
    ...
    local d_name=$(dirname ${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/is_unic_fn_name_.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: return 1 if bash fn name present in system
NAME: ${FNN}()
