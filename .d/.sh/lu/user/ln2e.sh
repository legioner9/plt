#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/ln2e.sh"
...
ln2e() {
    ...
    local d_name=$(dirname ${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/ln2e.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: echo all soft link in root dir {find . -type l -exec ls -la {} \;}
NAME: ${FNN}()
