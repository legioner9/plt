...
filename="${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/cr_cexa_e.sh"
...
cr_cexa_e() {
    ...
    local PPW=$(prs_f -d "${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/cr_cexa_e.sh") # FILENAME==="filename"
    path_ext=$PPW/$FNN
    ...
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FUNCNAME[0]}() help: 
            MAIN: create and edit ${PATH_VEXILLUM_DIR}/name._cexa in pwd or $1
            NAME: ${FUNCNAME[0]}()
            ARGS: [, $1: dest_dir]
            FLOW:${NORMAL}"
        return 0
