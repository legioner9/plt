#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/plt_vae.sh"
...
plt_vae() {
    ...
    local d_name=$(dirname ${PATH_COMMUNIS}/Deploy_store/.qa/lib/user/plt_vae.sh)
...
${NORMAL}"
    ...
    if "${ARGS[@]}"; then
        ...
        echo -e "${RED}--- fail exec: ${ARGS[*]} ---${NORMAL}" #exit 1
        plt_exit "fail exec: ${ARGS[*]}"
        return 1
    fi
