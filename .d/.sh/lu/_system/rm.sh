...
filename="$PATH_OS/.qa/lib/system/rm.sh"
...
aai_rm(){
    echo -e "${CYAN}---aai_rm() $*---${NORMAL}" #started functions
    
    arr=()
    res=()
    arr+=("rm -f или --force - Игнорировать несуществующие файлы и аргументы. Никогда не выдавать запросы на подтверждение удаления.")
    res+=("rm -f $*")
    arr+=("rm -i - Выводить запрос на подтверждение удаления каждого файла.")
    res+=("rm -i $*")
    arr+=("rm -I - Выдать один запрос на подтверждение удаления всех файлов, если удаляется больше трех файлов или используется рекурсивное удаление.")
    res+=("rm -I $*")
    arr+=("rm --interactive[=КОГДА] - Вместо КОГДА можно использовать:[never] — никогда не выдавать запросы [once] — выводить запрос один раз [always] — выводить запрос всегда")
    res+=("rm --interactive $*")
    arr+=("rm --one-file-system - Во время рекурсивного удаления пропускать директории, которые находятся на других файловых системах.")
    res+=("rm --one-file-system $*")
    arr+=("rm --no-preserve-root - Если в качестве директории для удаления задан корневой раздел /, то считать, что это обычная директория и начать выполнять удаление.")
    res+=("rm --no-preserve-root $*")
    arr+=("rm --preserve-root - Если в качестве директории для удаления задан корневой раздел /, то запретить выполнять команду rm над корневым разделом. Данное поведение используется по умолчанию.")
    res+=("rm --preserve-root $*")
    arr+=("rm -r или -R или --recursive - Удаление директорий и их содержимого. Рекурсивное удаление.")
    res+=("rm -r $*")
    arr+=("rm -d или --dir - Удалять пустые директории.")
    res+=("rm -d $*")
    arr+=("rm -v или --verbose - Выводить информацию об удаляемых файлах.")
    res+=("rm -v $*")
