#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/mtt_com__.ram/.grot/mtt_com__.sh"
...
mtt_com__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/mtt_com__.ram/.grot/mtt_com__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: open new mate-terminal[, with command \$1]
NAME: ${FNN}()
...
${NORMAL}"
    ...
    if [[ -z "$1" ]]; then
        ...
        mate-terminal --command="bash -c '$1;$SHELL'"
    fi
