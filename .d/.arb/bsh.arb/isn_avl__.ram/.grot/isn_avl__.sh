#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/isn_avl__.ram/.grot/isn_avl__.sh"
...
isn_avl__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/isn_avl__.ram/.grot/isn_avl__.sh)
...
${NORMAL}"
    ...
    if wget --spider -S "$1" 2>&1 | grep -w "200\|301\|302" >/dev/null; then
        return 1
    fi
