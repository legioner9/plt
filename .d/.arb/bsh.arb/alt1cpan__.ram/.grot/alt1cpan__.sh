#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/alt1cpan__.ram/.grot/alt1cpan__.sh"
...
alt1cpan__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/alt1cpan__.ram/.grot/alt1cpan__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: cpan in alt (cpan2rpm) install modules
NAME: ${FNN}()
...
${NORMAL}"
    ...
    sudo cpan2rpm --mk-rpm-dirs=~/rpm "$1"
