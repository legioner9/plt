#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/lsof_t__.ram/.grot/lsof_t__.sh"
...
lsof_t__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/lsof_t__.ram/.grot/lsof_t__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: echo only num ps at port \$1
NAME: ${FNN}()
