#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/tm_a__.ram/.grot/tm_a__.sh"
...
tm_a__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/tm_a__.ram/.grot/tm_a__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: tmux attach from name [, \$1]  
NAME: ${FNN}()
