#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/dk_al_cont__.ram/.grot/dk_al_cont__.sh"
...
dk_al_cont__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/dk_al_cont__.ram/.grot/dk_al_cont__.sh)
...
${NORMAL}"
    ...
    #{default_cntl_fn}
    # amount_arg $# 1 1
    echo -e "${HLIGHT}--- exec: docker ps -a -s ---${NORMAL}" #start files
    docker ps -a -s
    #{body_fn}
}
