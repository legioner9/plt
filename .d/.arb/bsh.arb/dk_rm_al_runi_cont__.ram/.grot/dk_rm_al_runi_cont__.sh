#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/dk_rm_al_runi_cont__.ram/.grot/dk_rm_al_runi_cont__.sh"
...
dk_rm_al_runi_cont__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/dk_rm_al_runi_cont__.ram/.grot/dk_rm_al_runi_cont__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: docker rm all runed contaner
NAME: ${FNN}()
