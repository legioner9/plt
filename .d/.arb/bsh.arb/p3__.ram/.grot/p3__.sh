#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/p3__.ram/.grot/p3__.sh"
...
p3__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/p3__.ram/.grot/p3__.sh)
...
${NORMAL}"
    ...
    # php -S localhost:${num_port} ${path_php_file} &
    mate-terminal --command="bash -c 'php -S localhost:${num_port} ${path_php_file};\
    $SHELL'"

    tm_k__ ${name_session}

    rm $(dirname ${path_php_file})/.$(basename ${path_php_file}).swp

    mate-terminal --command="bash -c 'tmux new -s ${name_session} -d;\
    tmux split-window -h -t ${name_session}:0.0;\
    tmux split-window -h -t ${name_session}:0.0;\
    
    tmux send-keys -t ${name_session}:0.0 \"vim ${path_php_file}\" Enter;\
    tmux send-keys -t ${name_session}:0.1 \"php ${path_php_file}\" Enter;\
    tmux send-keys -t \"${name_session}\":0.2 \"links2 http://localhost:${num_port}/\" Enter;\
