#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/tm_ka__.ram/.grot/tm_ka__.sh"
...
tm_ka__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/tm_ka__.ram/.grot/tm_ka__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: tmux kill all session
NAME: ${FNN}()
