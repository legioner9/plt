#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/dk_run_cont__.ram/.grot/dk_run_cont__.sh"
...
dk_run_cont__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/dk_run_cont__.ram/.grot/dk_run_cont__.sh)
...
${NORMAL}"
    ...
    #{default_cntl_fn}
    # amount_arg $# 1 1
    echo -e "${HLIGHT}--- exec: docker ps -s ---${NORMAL}" #start files
    docker ps -s
    #{body_fn}
}
