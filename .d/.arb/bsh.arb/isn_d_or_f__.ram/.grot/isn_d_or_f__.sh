#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/isn_d_or_f__.ram/.grot/isn_d_or_f__.sh"
...
isn_d_or_f__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/isn_d_or_f__.ram/.grot/isn_d_or_f__.sh)
...
${NORMAL}"
    ...
    if [ -z "$1" ]; then
        plt_info "in pzip_to_() : ERR_AMOUNT_ARGS : not define \$1: return 1"
        return 1
    fi
