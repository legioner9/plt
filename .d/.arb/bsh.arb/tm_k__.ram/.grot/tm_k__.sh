#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/.d/.arb/bsh.arb/tm_k__.ram/.grot/tm_k__.sh"
...
tm_k__() {
    ...
    local d_name=$(dirname ${PLT_PATH}/.d/.arb/bsh.arb/tm_k__.ram/.grot/tm_k__.sh)
    # wrp_fifs1_ cd ${d_name} -d
    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: tmux kill \$1 session
NAME: ${FNN}()
...
${NORMAL}"
    ...
    #{default_cntl_fn}
    # amount_arg $# 1 1
    echo -e "${HLIGHT}--- tmux kill-session -t $1 ---${NORMAL}" #start files
