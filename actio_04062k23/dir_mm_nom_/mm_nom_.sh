...
filename="${PLT_PATH}/actio/dir_mm_nom_/mm_nom_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_nom_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_nom_
    ...
    # >${FN_OUT_DIR}/com_mm_nom_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_nom_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_nom_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_nom_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_nom_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    res+=("echo bye)))")
    #{arr_res}
    arr+=("nom_f_ treat dir with .nom")
    res+=("echo nom_f_ is not complete")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
