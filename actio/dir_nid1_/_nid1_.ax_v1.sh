...
filename="${PLT_PATH}/actio/dir_nid1_/nid1_.sh"
...
nid1_() { #? $1 chaild function

    if [ -n "$1" ]; then
        local FNN="$1"
    else
        plt_exit "first arg function name is absent, second CNTL"
        return 1
    fi
    ...
    if [ -n "$2" ]; then
        ...
        plt_exit "first arg function name, second CNTL is absent"
        return 1
    fi
    # local FNN=${FUNCNAME[0]}
    # local ORIGO_DIR=$PATH_FN/dir_$FNN === ${FN_DIR}
    local FN_DIR=${PLT_PATH}/actio/dir_${FNN} #? contane dir of function
    local FLN=$0
    local ARGS=($@)
    local NARGS=$#
    # local ARGS1=$1
    ...
    # >${FN_OUT_DIR}/com_nid1_.res
    # COMMAND &>>${FN_OUT_DIR}/com_nid1_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_nid1_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_nid1_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_nid1_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
