...
if ! . "${FN_SAL_DIR}/ln2_.body.nsc.sh"; then
echo -e "${RED}--- fail: . ${FN_SAL_DIR}/ln2_.body.sh ---${NORMAL}"
return 1
fi
...
local ARGS=($@)
# echo ". ${FN_SAL_DIR}/${FNN}.body.nsc.sh"
if ! . "${PATH_SAL_AX_DIR}/ln2_.sal"; then
echo -e "${RED}--- fail: . ${PATH_SAL_AX_DIR}/ln2_.sal ---${NORMAL}"
return 1
fi
cd "$PPWD" 1>/dev/null || echo -e "${RED}--- fail : cd $PPWD ---${NORMAL}"
    #{init_body_init}
