#!/bin/bash
#. "${HOME}/.bashrc"
filename="${PLT_PATH}/actio/dir_fp_f_/_fp_f_/_mdeb/.cntl/fp_f_mdeb.sh"
...
fp_f_mdeb() {
    ...
    local d_name=$(dirname ${PLT_PATH}/actio/dir_fp_f_/_fp_f_/_mdeb/fp_f_mdeb.sh)

    #? cd in _fn dir
    wrp2_ cd --_xxd "${d_name}"

    #{intro_fn}
    if [ "-h" == "$1" ]; then
        echo -e "${CYAN} ${FNN}() help: 
MAIN: start all fn from _mdeb dir (use \"_head\" section as head point  )
NAME: ${FNN}()
