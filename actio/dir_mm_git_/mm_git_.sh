...
filename="${PLT_PATH}/actio/dir_mm_git_/mm_git_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_git_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_git_
    ...
    # >${FN_OUT_DIR}/com_mm_git_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_git_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_git_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_git_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_git_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
