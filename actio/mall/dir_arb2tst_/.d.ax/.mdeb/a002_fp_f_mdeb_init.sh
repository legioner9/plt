...
${NORMAL}"
    ...
    # amount_arg $# 1 1
    cd ${d_name} || {
        plt_info "in file://${PLT_PATH}/actio/mall/dir_arb2tst_/.d.ax/.mdeb/a002_fp_f_mdeb_init.sh : NOT_DIR : '${d_name}' return 1"
        return 1
    }
    local item
    local item2
    local arr_up_err=()
    IFS=$'\n'
    for item in $(f2e ${PLT_PATH}/actio/mall/dir_arb2tst_/.d.ax/.mdeb/.tmp/arb.lst); do
        # echo -e "${GREEN}\$item = $item${NORMAL}" #print variable

        for item2 in $(dd2e "$item"); do
            # echo -e "${GREEN}\${item2} = $item2${NORMAL}" #print variable
            if [ "${item2: -4}" == ".ram" ]; then
                # echo ${item}/${item2}/.grot
                arr_up_err+=(${item}/${item2}/.grot)
            fi
