...
filename="${PLT_PATH}/actio/mall/dir_mm_execf_/mm_execf_.nsc.sh"
...
mm_execf_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " mm_execf_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_mm_execf_/.d.ax/.sal.ax/mm_execf_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/mm_execf_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_mm_execf_/.d.ax/.sal.ax/mm_execf_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_mm_execf_/.d.ax/.sal.ax/mm_execf_.body.sh"
        return 1
    fi
