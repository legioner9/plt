...
filename="${PLT_PATH}/actio/mall/dir_uE_/uE_.nsc.sh"
...
uE_() {
    ...
    #     eval local $strex
    # done
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_uE_/.d.ax/.sal.ax/uE_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/uE_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_uE_/.d.ax/.sal.ax/uE_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_uE_/.d.ax/.sal.ax/uE_.body.sh"
        return 1
    fi
