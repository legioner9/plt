...
filename="${PLT_PATH}/actio/mall/dir_nm_opi_/nm_opi_.nsc.sh"
...
nm_opi_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " nm_opi_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_nm_opi_/.d.ax/.sal.ax/nm_opi_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_nm_opi_/.d.ax/.sal.ax/nm_opi_.body.sh"
        return 1
    fi
