...
filename="${PLT_PATH}/actio/mall/dir_f3e_/f3e_.nsc.sh"
...
f3e_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " f3e_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_f3e_/.d.ax/.sal.ax/f3e_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/f3e_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_f3e_/.d.ax/.sal.ax/f3e_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_f3e_/.d.ax/.sal.ax/f3e_.body.sh"
        return 1
    fi
