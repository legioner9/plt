...
filename="${PLT_PATH}/actio/mall/dir_fol_/fol_.nsc.sh"
...
fol_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " fol_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_fol_/.d.ax/.sal.ax/fol_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_fol_/.d.ax/.sal.ax/fol_.body.sh"
        return 1
    fi
