...
filename="${PLT_PATH}/actio/mall/dir_f2scr_/f2scr_.nsc.sh"
...
f2scr_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " f2scr_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_f2scr_/.d.ax/.sal.ax/f2scr_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_f2scr_/.d.ax/.sal.ax/f2scr_.body.sh"
        return 1
    fi
