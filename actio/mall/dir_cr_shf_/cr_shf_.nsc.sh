...
filename="${PLT_PATH}/actio/mall/dir_cr_shf_/cr_shf_.nsc.sh"
...
cr_shf_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " cr_shf_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_cr_shf_/.d.ax/.sal.ax/cr_shf_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/cr_shf_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_cr_shf_/.d.ax/.sal.ax/cr_shf_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_cr_shf_/.d.ax/.sal.ax/cr_shf_.body.sh"
        return 1
    fi
