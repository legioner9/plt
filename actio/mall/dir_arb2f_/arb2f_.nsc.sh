...
filename="${PLT_PATH}/actio/mall/dir_arb2f_/arb2f_.nsc.sh"
...
arb2f_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " arb2f_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for var_fn in $("${_f2e}" "${PLT_PATH}/actio/mall/dir_arb2f_/.d.ax/.sal.ax/arb2f_.local.list"); do
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_arb2f_/.d.ax/.sal.ax/arb2f_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_arb2f_/.d.ax/.sal.ax/arb2f_.body.sh"
        return 1
    fi
