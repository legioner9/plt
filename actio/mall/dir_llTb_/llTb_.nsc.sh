...
filename="${PLT_PATH}/actio/mall/dir_llTb_/llTb_.nsc.sh"
...
llTb_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " llTb_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_llTb_/.d.ax/.sal.ax/llTb_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/llTb_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_llTb_/.d.ax/.sal.ax/llTb_.body.nsc.sh"; then
        # ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_llTb_/.d.ax/.sal.ax/llTb_.body.sh"
        return 1
    fi
