...
filename="${PLT_PATH}/actio/mall/dir_bdpl_pzip_/bdpl_pzip_.nsc.sh"
...
bdpl_pzip_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " bdpl_pzip_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for var_fn in $("${_f2e}" "${PLT_PATH}/actio/mall/dir_bdpl_pzip_/.d.ax/.sal.ax/bdpl_pzip_.local.list"); do
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_bdpl_pzip_/.d.ax/.sal.ax/bdpl_pzip_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_bdpl_pzip_/.d.ax/.sal.ax/bdpl_pzip_.body.sh"
        return 1
    fi
