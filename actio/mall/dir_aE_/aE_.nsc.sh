...
filename="${PLT_PATH}/actio/mall/dir_aE_/aE_.nsc.sh"
...
aE_() {
    ...
    for var_fn in $("${_f2e}" "${PLT_PATH}/actio/mall/dir_aE_/.d.ax/.sal.ax/aE_.local.list"); do
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_aE_/.d.ax/.sal.ax/aE_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_aE_/.d.ax/.sal.ax/aE_.body.sh"
        return 1
    fi
