...
filename="${PLT_PATH}/actio/mall/dir_el_f_h_/el_f_h_.nsc.sh"
...
el_f_h_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " el_f_h_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_el_f_h_/.d.ax/.sal.ax/el_f_h_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/el_f_h_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_el_f_h_/.d.ax/.sal.ax/el_f_h_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_el_f_h_/.d.ax/.sal.ax/el_f_h_.body.sh"
        return 1
    fi
