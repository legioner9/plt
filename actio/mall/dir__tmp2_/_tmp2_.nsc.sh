...
filename="${PLT_PATH}/actio/mall/dir__tmp2_/_tmp2_.nsc.sh"
...
_tmp2_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " _tmp2_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir__tmp2_/.d.ax/.sal.ax/_tmp2_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/_tmp2_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir__tmp2_/.d.ax/.sal.ax/_tmp2_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir__tmp2_/.d.ax/.sal.ax/_tmp2_.body.sh"
        return 1
    fi
