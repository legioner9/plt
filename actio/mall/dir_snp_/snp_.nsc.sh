...
filename="${PLT_PATH}/actio/mall/dir_snp_/snp_.nsc.sh"
...
snp_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " snp_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_snp_/.d.ax/.sal.ax/snp_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/snp_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_snp_/.d.ax/.sal.ax/snp_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_snp_/.d.ax/.sal.ax/snp_.body.sh"
        return 1
    fi
