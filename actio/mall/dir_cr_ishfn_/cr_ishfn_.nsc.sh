...
filename="${PLT_PATH}/actio/mall/dir_cr_ishfn_/cr_ishfn_.nsc.sh"
...
cr_ishfn_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " cr_ishfn_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for var_fn in $("${_f2e}" "${PLT_PATH}/actio/mall/dir_cr_ishfn_/.d.ax/.sal.ax/cr_ishfn_.local.list"); do
    ...
    if ! . "${PLT_PATH}/actio/mall/dir_cr_ishfn_/.d.ax/.sal.ax/cr_ishfn_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_cr_ishfn_/.d.ax/.sal.ax/cr_ishfn_.body.sh"
        return 1
    fi
