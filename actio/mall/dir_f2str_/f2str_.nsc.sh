...
filename="${PLT_PATH}/actio/mall/dir_f2str_/f2str_.nsc.sh"
...
f2str_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " f2str_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_f2str_/.d.ax/.sal.ax/f2str_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/f2str_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_f2str_/.d.ax/.sal.ax/f2str_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_f2str_/.d.ax/.sal.ax/f2str_.body.sh"
        return 1
    fi
