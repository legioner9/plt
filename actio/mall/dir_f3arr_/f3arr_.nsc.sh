...
filename="${PLT_PATH}/actio/mall/dir_f3arr_/f3arr_.nsc.sh"
...
f3arr_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " f3arr_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_f3arr_/.d.ax/.sal.ax/f3arr_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/f3arr_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_f3arr_/.d.ax/.sal.ax/f3arr_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_f3arr_/.d.ax/.sal.ax/f3arr_.body.sh"
        return 1
    fi
