...
filename="${PLT_PATH}/actio/mall/dir_bcp_l2d_/bcp_l2d_.nsc.sh"
...
bcp_l2d_() {
    ...
    if ! ${_garg2e2_} "${ARGS[@]}"; then
        ${_plt_exit} " bcp_l2d_ return 1: ${FNLOCK}"
        return 1
    fi
    ...
    for local_var_fn in $(f2e "${PLT_PATH}/actio/mall/dir_bcp_l2d_/.d.ax/.sal.ax/bcp_l2d_.local.list"); do
    ...
    # echo ". ${FN_SAL_DIR}/bcp_l2d_.body.nsc.sh"
    if ! . "${PLT_PATH}/actio/mall/dir_bcp_l2d_/.d.ax/.sal.ax/bcp_l2d_.body.nsc.sh"; then
        ${_plt_exit} "fail: . ${PLT_PATH}/actio/mall/dir_bcp_l2d_/.d.ax/.sal.ax/bcp_l2d_.body.sh"
        return 1
    fi
