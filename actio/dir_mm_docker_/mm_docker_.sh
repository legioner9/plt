...
filename="${PLT_PATH}/actio/dir_mm_docker_/mm_docker_.sh"
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_docker_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_docker_
    ...
    # >${FN_OUT_DIR}/com_mm_docker_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_docker_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_docker_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_docker_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_docker_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
    ...
    res+=("echo bye)))")
    #{arr_res}
    arr+=("add user st to docker group")
    res+=("sudo usermod -aG docker")
    arr+=("restart docker")
    res+=("systemctl enable --now docker")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
    # arr+=("aaaaaaaaa")
    # res+=("bbbbbbbbb")
