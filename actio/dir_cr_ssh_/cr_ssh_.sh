...
filename="${PLT_PATH}/actio/dir_cr_ssh_/cr_ssh_.sh"
...
#{see_cntl}
exit_err_cr_ssh_() { #* $1 error message
local arg1="$1"
unset_autoargs_cr_ssh_
...
# >${FN_OUT_DIR}/com_cr_ssh_.res
# COMMAND &>>${FN_OUT_DIR}/com_cr_ssh_.res
# if [ $? -ne 0 ]; then
#     if [ 1 -eq ${verbose} ]; then
#         cat ${FN_OUT_DIR}/com_cr_ssh_.res
#     fi
#     #? runtime error allowed locally
#     error_allowed_locall_string="STRING"
#     if cat ${FN_OUT_DIR}/COMMAND_cr_ssh_.res | is_rl_siev_and ${error_allowed_locall_string}; then
#         continue
#     else
#         exit_err_cr_ssh_ "runtime error:: COMMAND ${addr}"
#     fi
# fi
#! amount args
