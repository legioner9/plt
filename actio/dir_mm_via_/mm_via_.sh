...
filename="${PLT_PATH}/actio/dir_mm_via_/mm_via_.sh"
...
MAIN: exec ener.sh in menu as list dir in .via
NAME: ${FNN}()
...
${NORMAL}"
    ...
    #{see_cntl}
    exit_err_mm_via_() { #* $1 error message
        local arg1="$1"
        unset_autoargs_mm_via_
    ...
    # >${FN_OUT_DIR}/com_mm_via_.res
    # COMMAND &>>${FN_OUT_DIR}/com_mm_via_.res
    # if [ $? -ne 0 ]; then
    #     if [ 1 -eq ${verbose} ]; then
    #         cat ${FN_OUT_DIR}/com_mm_via_.res
    #     fi
    #     #? runtime error allowed locally
    #     error_allowed_locall_string="STRING"
    #     if cat ${FN_OUT_DIR}/COMMAND_mm_via_.res | is_rl_siev_and ${error_allowed_locall_string}; then
    #         continue
    #     else
    #         exit_err_mm_via_ "runtime error:: COMMAND ${addr}"
    #     fi
    # fi
    #! amount args
