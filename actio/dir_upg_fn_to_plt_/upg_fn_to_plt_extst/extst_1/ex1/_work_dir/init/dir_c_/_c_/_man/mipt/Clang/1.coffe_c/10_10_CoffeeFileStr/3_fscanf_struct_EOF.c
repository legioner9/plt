...
void ai_cpy(Ai *dest, Ai *src)
{
    dest->x = src->x;
    dest->y = src->y;
}

int get_struct_from_fill(FILE *fd, Ai *sarr)
{
    Ai tmp;
    int numerus = 0;
    while (fscanf(fd, "%d%d", &(tmp.x), &(tmp.y)) != EOF)
    {
        ai_cpy(&sarr[numerus], &tmp);
        numerus++;
    }
         ...
         char **envp)  // Array of environment variable strings
{
    ...
    if ((fdn = fopen("list_eof.fill", "r")) == NULL)
    {
        perror("list_eof.fill not open:");
        exit(1);
    }
    ...
    width_sarr = get_struct_from_fill(fdn, sarr);
    printf("width sarr: %d\n", width_sarr);
