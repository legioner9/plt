...
/*#*/ int mkfifo_с(const char *file_path, mode_t mode, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        int a = mkfifo(file_path, mode);
        if (-1 == a)
        {
            if (st__mo & STMO_PRN)
            {
                fprintf(stderr, "failed in file %s at line # %d\n", file, line);
                fprintf(stderr, "from mkfifo_с file_path = %s, errno = %d\n", file_path, errno);
                perror("Error is: ");
                fflush(stdout);
            }
            if (st__re & STRE_MNS)
...
/*#*/ int mknod_с(char *path, mode_t mode, dev_t dev, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        int a = mknod(path, mode, dev);
        if (-1 == a)
        {
            if (st__mo & STMO_PRN)
            {
                fprintf(stderr, "failed in file %s at line # %d\n", file, line);
                fprintf(stderr, "from mknod_с path = %s,mode = %d, dev = %d errno = %d\n", path, mode, dev, errno);
                perror("Error is: ");
                fflush(stdout);
            }
            if (st__re & STRE_MNS)
...
/*#*/ int stat_с(const char *restrict path, struct stat *restrict buf, const char *file, const int line, int st__mo, int st__if, int st__re)
{
    ...
    if (st__if & STIF_ONE)
    {
        ...
        if (-1 == a)
        {
            ...
            {
                fprintf(stderr, "failed in file %s at line # %d\n", file, line);
                fprintf(stderr, "from stat_с path = %s, errno = %d\n", path, errno);
                perror("Error is: ");
                fflush(stdout);
            }
            if (st__re & STRE_MNS)
