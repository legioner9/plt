...
var url = 'https://c.tmyzer.com/c/?s=82332&f=19&fi=99';
...
delete Array.prototype.removeElement;
            ...
            var refreshVisibility26711 = function(){
                        var observers = [];
            ...
            var invisibleRefreshRate =47000;
            ...
            //window.adsArea26711= el;
            window.adsArea26711.isVisible = false;
            window.adsArea26711.refreshTimer = Date.now();
            window.adsArea26711.formatId = adsAreaId;
            window.adsArea26711.refreshRate = refreshTimeTableVisible[adsAreaId];
            window.adsArea26711.capping = 1;
            window.adsArea26711.cappingLimit = adsCappingLimit;
            ...
            if (Math.floor(entry.intersectionRatio * 100) > 50 && !window.adsArea26711.isVisible) {
            window.adsArea26711.isVisible = true;
            window.adsArea26711.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (Math.floor(entry.intersectionRatio * 100) < 50 && window.adsArea26711.isVisible) {
            window.adsArea26711.isVisible = false;
            window.adsArea26711.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            });
            };

            observers26711 = new IntersectionObserver(callback, option, true, 1000);

                            observers26711.observe(document.querySelector("#sas_" + adsAreaId));
                        } else {
            window.adsArea26711.lastPos = 0;
            window.adsArea26711.lastTime = Date.now();
            evt.scroll = window.addEventListener('scroll', function (e) {
            if ((Math.abs(window.adsArea26711.lastPos - window.scrollY) > 100) || (Date.now() - window.adsArea26711.lastTime > 200)) {
            window.adsArea26711.lastPos = window.scrollY;
            window.adsArea26711.lastTime = Date.now();
            var visibilitySensor = true;
            el = document.querySelector('#sas_26711')
            ...
            if (visibilitySensor && !window.adsArea26711.isVisible) {
            window.adsArea26711.isVisible = true;
            window.adsArea26711.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (!visibilitySensor && window.adsArea26711.isVisible) {
            window.adsArea26711.isVisible = false;
            window.adsArea26711.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            }
            ...
            refreshQueueManager26711 = function (formatId, capping) {
            window.adsArea26711.capping++;
            window.adsArea26711.refreshTimer = Date.now();

            var isClosed = false;
            if(window.adsArea26711.isClosed != undefined){
                if(window.adsArea26711.isClosed){
                    isClosed = true
                }
            }
            ...
            if(formatId == 26328){
                if(isClosed == false){
                    refreshSlotFooter(26711, window.adsArea26711.capping);
                }
            }else{
            refreshSlot(26711, window.adsArea26711.capping,window.adsArea26711.isVisible);
            }
            }
            setInterval(function(){loopChecker26711();}, 1000);
            loopChecker26711 = function() {
            if (typeof tmzr.getAllWinningBids == 'undefined') {return}
                el = window.adsArea26711;
                if (!isNaN(el.refreshRate) && el.capping < el.cappingLimit && el.formatId != '26328') {
                    lastBidder26711 = {"bidderCode":undefined, "responseTimestamp":0};
                    tmzr.getAllWinningBids().forEach(function(bid){
                        if(bid.adUnitCode==el.formatId){
                            if(bid.responseTimestamp > lastBidder26711.responseTimestamp){
                                lastBidder26711 = bid;
                            }
                        }
                    })
                    window.adsArea26711.lastBidder = lastBidder26711;
                    if(slowBidders.indexOf(el.lastBidder.bidder)==-1){
                        slowdown = 0;
                    } else {
                        slowdown = parseInt(slowBiddersTimer[slowBidders.indexOf(el.lastBidder.bidder)]);
                    }
                    if ((window.adsArea26711.refreshTimer + el.refreshRate) < Date.now()) {
                        if(((el.refreshTimer + slowdown) < Date.now()) && slowBidders.includes(el.lastBidder.bidder) == true) {
                            refreshQueueManager26711(el.formatId, el.capping);
                    ...
                    if (window.adsArea26711.refreshTimer + 25000 < Date.now() && !slowBidders.includes(el.lastBidder.bidder)) {
                        refreshQueueManager26711(el.formatId, el.capping);
                    } else if ((window.adsArea26711.refreshTimer + slowdown < Date.now() && slowBidders.includes(el.lastBidder.bidder))) {
                        refreshQueueManager26711(el.formatId, el.capping);
                    }
                ...
                var mydiv = document.getElementById("82332-19");
                var creatediv = document.createElement("div");
                creatediv.id = "sas_26711";
                
                if(19 == 2 && 1 == 1 && ((navigator.userAgent.match(/Mobile|iP(hone|od|ad)|Android|BlackBerry|IEMobile/i))||(typeof window.md !== 'undefined' && window.md.phone() !== null))){
                ...
                var arr_width_video ={26300:300,26711:300,26323:300,39287:444,45111:640};
                var arr_height = {26322:90,26300:250,26323:600,26324:600,26711:250,26706:600,28108:150,30012:90,39287:250,35:50677,51410:90,28108:250};
                var bid = tmzr.getHighestCpmBids("26711")[0];
                if(bid){
                var hb_adid = window.targetingParams[26711].hb_adid;
                var sasBid = JSON.parse(JSON.stringify(bid));
                sasBid.cpm = window.targetingParams[26711].hb_bid;
                bid.currency = bid.currency || "USD";
                }else{
                var sasBid ={};
                }
                ...
                console.log();
                sas.setHeaderBiddingWinner("sas_"+26711, sasBid);
                                sas.call("std", {
                tagId: 'sas_26711',
                siteId: 435043,
                pageId: 1420944,
                ...
                onNoad: function() {
                    if (bid && bid.bidder == "improvedigital" && bid.mediaType == "video") {
                        var creatediv = document.createElement("div");
                        creatediv.id = '26711';
                        creatediv.setAttribute("style","width:"+arr_width_video[26711]+"px;margin:auto;");
                        document.getElementById("sas_26711").appendChild(creatediv);
                        var iframeDoc = document;
                    }else{
                        var myiframe = document.createElement('iframe');
                        myiframe.frameBorder=0;
                        myiframe.width=arr_width[26711]+"px";
                        myiframe.height=arr_height[26711]+"px";
                        myiframe.id="sas_iframe_26711";
                        myiframe.setAttribute("scrolling", "no");
                        myiframe.setAttribute("marginheight", 0);
                        myiframe.setAttribute("marginwidth", 0);
                        myiframe.setAttribute("topmargin", 0);
                        myiframe.setAttribute("leftmargin", 0);
                        myiframe.setAttribute("allowtransparency", true);
                        document.getElementById("sas_26711").appendChild(myiframe);
                        var iframeDoc = myiframe.contentWindow.document;
                    }
                                            if (bid && bid.mediaType != "native") {
                            tmzr.renderAd(iframeDoc, hb_adid);
                ...
                var http = new XMLHttpRequest();
                var url = 'https://c.tmyzer.com/c/?s=82332&f=19&fi=0';
                http.open("GET", url, true);
                http.send();
